...
Abstract:
    ...
    This file contains helpers for using MsQuic, entirely with the public API.
...
#ifndef _MSQUICPUBLICHELPER_
#define _MSQUICPUBLICHELPER_
...
struct CreateSecConfigHelper {
        ...
        )
    {
        ...
        Promise->set_value(SecurityConfig);
    }
    ...
    {
        std::promise<QUIC_SEC_CONFIG*> SecurityConfigurationPromise;
        if (QUIC_SUCCEEDED(
            MsQuic->SecConfigCreate(
                ...
                Certificate,
                Principal,
                &SecurityConfigurationPromise,
                GetSecConfigComplete))) {
            return SecurityConfigurationPromise.get_future().get();
        }
        return nullptr;
    }
};
    ...
    )
{
    ...
    std::memset(&CertHashStore, 0, sizeof(CertHashStore));
